...
- **mode** (`feature` only — a `bug` is always full because it needs a regression test) —
  `full` (default) / `lite`. Suggest `lite` for an obviously trivial, **behavior-neutral** ask
  (copy/text, CSS, a trivial config tweak) that touches few files, changes no schema/API, and
  isn't security-sensitive; the user can set or veto it. This is only a hint — the **manager
  re-checks the lite gate** and silently runs full (or auto-promotes mid-flight) if it
  doesn't qualify, so a wrong guess is caught, not relied on. Record `mode: lite` in the
  card; omit the field for full.

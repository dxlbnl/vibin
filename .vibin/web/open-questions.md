...
- **Capture mechanism** ([capture](capture.md)) — hook-harvested (observe → candidates) vs a forced cheap step at
  close? Automatic is the bet, but how automatic?
...
**Resolved:** knowledge lives in **wiki atoms only, never native memory** (opaque) — see
[what belongs](what-belongs.md). Superseded atoms are **deleted and repointed**, not kept — see
[the knowledge graph](the-graph.md).

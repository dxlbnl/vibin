...
| v10 | 2026-05-24 | Agents **search with the `Grep`/`Glob`/`Read` tools, not shelled Bash** (`grep`/`find`/`rg`/`cat`) — the tools are auto-allowed and silent, while a shelled search forces a permission prompt. CLAUDE.md rule + a section in the Bash-capable agents (`reviewer`, `test-writer`, `implementer`). | [`migrations/0010-search-with-tools.md`](migrations/0010-search-with-tools.md) |
| v11 | 2026-05-24 | **Doc reconcile** to the post-0005 needs-answers model — fixes a self-contradiction in `spec-writer` (anatomy still said "set flags: [review]" while "Your task" said the manager bounces with `needs-answers`) and the same drift in `wiki/INDEX.md`, the wiki skill, and the `specs/README.md` template placeholder. Adds `needs-answers` to the documented flags lists and `browser-testing` to the manager's Practices prose enumeration. | [`migrations/0011-reconcile-open-questions-docs.md`](migrations/0011-reconcile-open-questions-docs.md) |

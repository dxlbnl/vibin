...
test-first** multi-agent workflow. These rules are enforced by hooks in
`.claude/` and by the agent definitions in `.claude/agents/`.
...
- **Top-level boundary** — the top-level session answers questions and runs
  `/bootstrap`. Build work goes through the `manager` pipeline; the top-level session
...
- **Run until blocked** — the manager works through the backlog without per-item
  ...
  escalation.
- **Review checkpoints** — the manager pauses (by returning to the top-level session
  for approval, then resuming) for: (1) the initial work plan, always; (2) any
  `wiki/backlog.md` item flagged `review`. Items are flagged by the user or auto-flagged
  by the manager when risky/ambiguous/architecturally significant. Unflagged items never
  pause.
...
- **Specialist agents** — beyond the core five, the manager may spawn ad-hoc
  `general-purpose` specialists (researcher, security-auditor, designer, …) or persist
  recurring ones as `.claude/agents/*.md`. All specialists obey the same rules: read the
  wiki first, hook-gated, artifact handoff.
